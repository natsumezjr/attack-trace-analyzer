...
- `run/client-*/data/` 下的：
  ...
  - filebeat 输出（如 `filebeat-output/`）
- RabbitMQ 队列数据（最简单做法：重建 rabbitmq 容器）

验收标准：

- 重新启动 client 后，RabbitMQ 队列会重新产生消息
- client API 的 `total` 在做动作后会出现 `>0` 的返回（且消费后可归零）
...
并确保每个实例使用不同数据目录，否则输出文件会混在一起（排障极难）。
