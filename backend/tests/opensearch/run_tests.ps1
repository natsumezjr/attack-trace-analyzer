...
try {
    ...
    Write-Host "  cd backend; docker compose up -d opensearch"
    exit 1
}

# 进入 backend 目录（仓库根目录下的 backend\）
$RepoRoot = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $ScriptDir))))
