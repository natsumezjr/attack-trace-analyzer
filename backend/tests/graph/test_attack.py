...
from app.services.neo4j import get_graph_by_attack_id
...
def _load_attack_events() -> list[dict[str, Any]]:
    fixture_path = _fixtures_dir() / "graph" / "attack.json"
...
def test_ingest_attack_fixture_into_neo4j() -> None:
...
@pytest.mark.requires_neo4j
def test_get_graph_by_attack_id_returns_edges_and_nodes() -> None:
    events = _load_attack_events()
    eligible_ids = _eligible_event_ids(events)
    host_ids = _collect_host_ids(events)
    user_ids = _collect_user_ids(events)
    ...
    try:
        ...
        nodes, edges = get_graph_by_attack_id("T1059")
        assert nodes
        assert edges
        assert all(edge.props.get("threat.technique.id") == "T1059" for edge in edges)

        nodes, edges = get_graph_by_attack_id("TA0001")
        assert nodes
        assert edges
        assert all(edge.props.get("threat.tactic.id") == "TA0001" for edge in edges)
    finally:
        if not keep_data:
            _delete_edges_by_event_ids(eligible_ids)
            _delete_nodes_by_host_and_user(host_ids, user_ids)
