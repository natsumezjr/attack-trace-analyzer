...
try:
    ...
    from neo4j import db as graph_api  # type: ignore
    from neo4j import ingest  # type: ignore
...
def main() -> None:
    if "--file" in sys.argv:
        ...
        node_count, edge_count = ingest.ingest_ecs_events(events)
    else:
        # 通过 OpenSearch 拉取并入图
        total_events, node_count, edge_count = ingest.ingest_from_opensearch()
