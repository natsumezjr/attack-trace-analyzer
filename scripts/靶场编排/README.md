...
本目录包含攻击溯源分析系统的靶场编排脚本和配置文件，用于项目演示和汇报。

**目录说明**：
- 本目录提供**快速演示工具**（2-3 分钟完成攻击模拟）
- 如需更详细的 APT 攻击模拟（12 阶段），请参考 `../apt_simulation/`

## 目录结构

```
靶场编排/
├── README.md          # 本文件：靶场编排说明
├── start.sh           # 靶场一键启动脚本
├── close.sh           # 靶场一键关闭脚本
├── attack_basic.sh    # 基础攻击模拟脚本（6步，良性行为）
├── check_ports.sh     # 端口检查工具
├── c2_config/         # C2服务配置文件（DNS+HTTP）
│   ├── docker-compose.yml
│   ├── coredns/
│   └── nginx/
└── 常用命令.md         # 常用运维命令清单
...
**相关工具**：
- 详细 APT 模拟工具：`../apt_simulation/`（12 阶段完整攻击链）
- Scripts 总览：`../README.md`
...
### 3. attack_basic.sh - 基础攻击模拟脚本

**功能**：模拟基础攻击链（良性行为），用于生成可观测的安全事件证据

**攻击链步骤**（与 `docs/20-需求与验收/22-攻击场景与复现剧本.md` 完全对齐）：
...
**与详细攻击模拟的区别**：
- 本脚本：6 步基础攻击，适合快速演示（2-3 分钟）
- 详细脚本：12 阶段完整 APT 攻击，适合深入教学（见 `../apt_simulation/`）
...
### 靶场相关
- 攻击剧本定义：`../../docs/20-需求与验收/22-攻击场景与复现剧本.md`
