...
- `include_nodes`：为 `true` 时，响应中同时返回 `nodes[]`（按边集合中的 UID 去重拉取）。
...
当请求中 `include_nodes=true` 时，响应中返回 `nodes[]`。每个节点对象结构固定为：
...
## 5. TTP 相似度接口：`POST /api/v1/analysis/ttp-similarity`

### 5.1 Request Body（固定字段）
