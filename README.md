...
- **TTP 知识库**: 离线 ATT&CK Enterprise CTI（STIX 2.1｜attack-stix-data｜默认路径：`backend/app/services/ttp_similarity/cti/enterprise-attack.json`，可用 `ATTACK_CTI_PATH` 覆盖）。数据文件较大，默认不提交到 Git；可用 `backend/scripts/fetch_mitre_attack_cti.sh` 一键下载。
