...
**结论（v1）**：本项目采用 **OpenSearch**（索引：`client-registry-*`）；SQLite/PG 仅作为备选讨论或报告补充，不作为 v1 落地口径。
...
| 索引 | 保留周期 | 删除策略 | 备份建议 |
|------|---------|---------|---------|
| `ecs-events-*` | 30 天 | 到期删除（ILM） | 可选归档 |
| `raw-findings-*` | 180 天 | 到期删除（ILM） | 建议归档 |
| `canonical-findings-*` | 180 天 | 到期删除（ILM） | 可选备份 |
...
> **说明**：本项目 v1 的注册表/轮询状态落 OpenSearch（`client-registry-*`）。本节仅用于：  
> 1) 课程报告中展示传统"数据库表设计"；或  
> 2) 作为备选方案（若后续需要用 SQLite/PG 管理元数据）。
