...
>
> **相关文档**：
> - ECS 字段规范（Telemetry/Findings 统一口径）：`docs/03A-ECS字段规范.md`
> - 客户端注册与中心轮询接口：`docs/03C-客户端中心机接口规范.md`
> - 数据设计总览：`docs/03-数据库设计.md`
...
- [4. （可选）关系型数据库表结构](#4-可选关系型数据库表结构用于报告或中心机元数据)
...
**分层存储**：
- Telemetry：量大，短期保留（7–30 天）
- Raw Findings：量中等，中期保留（30–180 天）
- Canonical Findings：量小，长期保留（30–180 天或更长）
...
| 索引模式 | 数据对象 | 保留周期 | 数据量 |
|---------|---------|---------|:----:|
| `ecs-events-*` | Telemetry | 7–30 天 | ⭐⭐⭐⭐⭐ |
| `raw-findings-*` | Raw Findings | 30–180 天 | ⭐⭐⭐⭐ |
| `canonical-findings-*` | Canonical Findings | 30–180 天+ | ⭐⭐⭐ |
...
字段语义与"各 dataset 的字段要求"统一以 `docs/03A-ECS字段规范.md` 为准。本节仅给出 **索引模板（mapping）层面的要点**。
...
| `poll.last_error` | `text` | 最后错误（可选） | `Connection refused` |
...
| 方案 | 优点 | 缺点 | 适用场景 |
|------|------|------|---------|
| **OpenSearch** | 查询方便，与其他数据统一 | 占用存储 | 演示/查询频繁 |
| **SQLite/PG** | 轻量，事务支持好 | 需要额外维护 | 生产环境 |

**说明**：如果使用 Prisma/SQLite 维护注册表，也可以不落 OpenSearch，只保留关系表（见第 4 节）。
...
| 索引 | 保留周期 | 删除策略 | 备份建议 |
|------|---------|---------|---------|
| `ecs-events-*` | 7–30 天 | 按日期删除 | 可选归档 |
| `raw-findings-*` | 30–180 天 | 按日期删除 | 建议归档 |
| `canonical-findings-*` | 30–180 天+ | 长期保留 | 建议备份 |
| `attack-chains-*` | 长期 | 不删除 | 强烈建议备份 |
| `client-registry-*` | 长期 | 不删除 | 建议备份 |
| **Neo4j** | 长期 | 不删除 | 强烈建议备份 |

**说明**：
- 可使用 OpenSearch **ILM（Index Lifecycle Management）** 自动管理
- 不同生命周期的数据可设置不同的保留策略
- Neo4j 用于路径重建与展示，建议长期保留
...
| **多跳查询** | 支持路径重建：初始入侵 → 执行 → 横向 → C2 → 外传 | Cypher 查询 |
...
| `Endpoint`（可选） | 网络端点 | `addr` | - | `addr: "10.0.0.5:443/tcp"` |
...
| 关系 | 说明 | 属性 | 示例 |
|------|------|------|------|
| `(User)-[:LOGON]->(Host)` | 用户登录主机 | `ts`, `custom_evidence_event_ids` | alice 登录 victim-01 |
| `(Process)-[:SPAWNED]->(Process)` | 进程启动进程 | `ts`, `custom_evidence_event_ids` | bash 启动 ssh |
| `(Process)-[:ACCESSED]->(File)` | 进程访问文件 | `op`, `ts`, `custom_evidence_event_ids` | ssh 读取 /etc/passwd |
| `(Process)-[:CONNECTED]->(IP)` | 进程连接 IP | `ts`, `dst_port`, `custom_evidence_event_ids` | curl 连接 10.0.0.5:443 |
| `(Host)-[:RESOLVED]->(Domain)` | 主机解析域名 | `ts`, `custom_evidence_event_ids` | victim-01 解析 evil.com |
| `(Domain)-[:RESOLVES_TO]->(IP)` | 域名解析到 IP | `ts`, `custom_evidence_event_ids` | evil.com → 10.0.0.5 |
...
2. 在 Neo4j 中抓取对应时间窗/邻域子图并做路径搜索
3. 输出攻击链对象写回 `attack-chains-*`，并附带 Neo4j 子图引用（查询参数/节点 ID 列表等）
...
3. Neo4j 路径搜索：
   MATCH path = (h:Host {id: "h-xxx"})<-[:SPAWNED*]-(p:Process)
   WHERE p.start_ts >= "2026-01-12T00:00:00Z"
   RETURN path
...
> **说明**：若需要在课程报告中提供传统"数据库表设计"，或你们中心机希望用 SQLite/PG 管理注册表，可按下述结构描述。
...
- **ECS 字段规范**：`docs/03A-ECS字段规范.md`
- **客户端中心机接口规范**：`docs/03C-客户端中心机接口规范.md`
- **数据设计总览**：`docs/03-数据库设计.md`
- **模块与数据流说明**：`docs/04-模块与数据流说明.md`
