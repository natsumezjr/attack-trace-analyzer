...
这些变量由 `client/docker-compose.yml` 固定注入，用于容器内部读写路径与表名：

- `DB_PATH`：Filebeat detector 写入 SQLite 的路径
- `DB_FILE`：Suricata exporter 写入 SQLite 的路径
- `TABLE_NAME`：写入 SQLite 的表名（`filebeat`/`suricata`/`falco`）
