...
- 取离线 MITRE ATT&CK Enterprise CTI（STIX 2.1｜attack-stix-data｜默认路径：`backend/app/services/ttp_similarity/cti/enterprise-attack.json`，可用 `ATTACK_CTI_PATH` 覆盖）数据包中的 Intrusion Set（组织）→ technique 集合，生成每个组织的向量表示 `V_i`（数据文件可用 `./scripts/fetch_attack_cti.sh` 获取/更新）。
