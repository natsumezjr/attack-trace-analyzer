...
- **客户机模块**：Filebeat(+Sigma)/Falco/Suricata 采集数据，由 **Go 客户端后端（SQLite 缓冲）**统一归一化/缓冲，并向中心机注册（注册后等待中心轮询拉取）。  
- **中心机模块**：以 **Next.js 全栈**为核心，维护客户端注册表与轮询状态；拉取数据后写入 OpenSearch/Neo4j，并运行关联算法生成攻击链，最终由前端展示与导出。

> 模块与数据流故事线见：`docs/05-模块与数据流说明.md`  
> 客户端注册与中心轮询接口见：`docs/06C-客户端中心机接口规范.md`

### 1.1 组件分层

1) **客户机采集层（Sensors）**  
主机系统日志/主机行为/网络流量分别由对应传感器采集（本项目固定为 Filebeat(+Sigma)/Falco/Suricata）。

2) **客户端后端层（Client Backend / Go）**  
把不同来源事件映射到 ECS 子集字段并 **SQLite 本地缓冲**；启动后向中心机注册；对外提供拉取接口供中心机轮询。

3) **中心机后端服务（Center Backend / FastAPI）**
接收客户端注册、维护注册表与 cursor；按 interval 轮询拉取数据；提供查询 API；入库路由；触发关联分析算法。

4) **中心机前端服务（Center Frontend / Next.js + React）**
提供页面展示、可视化、报告导出等用户界面。

5) **存储与检索层（Storage）**
以 **OpenSearch + Neo4j** 为核心存储：OpenSearch 保存 Telemetry/Findings/Chains；Neo4j 保存实体关系图。

6) **检测与关联层（Detection & Correlation）**
...
- 主机系统日志：Filebeat（主机系统日志采集使用 Filebeat；异常检测使用 Sigma 规则）
- 主机行为：Falco（输出行为告警为主）
...
>
> 说明：本项目以 ECS 为主（`event.* / rule.* / threat.* / host.* ...`），所有自定义字段统一放入 `custom.*` 命名空间（见 `docs/06A-ECS字段规范.md`）。
...
> 说明：node/edge 的权威规格与抽取口径以 `docs/09-实体图谱节点与关系规范v2.md` 为准；本节仅做系统规格层面的简表描述。
...
3) 从“初始入侵候选点”（外联入口/可疑登录/漏洞利用告警）出发，在图上做路径搜索：
   - 采用 **Neo4j GDS 加权最短路**在实体关系图上做路径搜索
   - 图投影约束：按时间窗 Δt 过滤边，并限定关系类型（进程链 / 登录会话 / 网络外联 / DNS 解析 / 文件访问）
   - 边权重由“关联强度”决定：强关联（进程父子、同一 `process.entity_id`）权重更低；弱关联（仅 IP/domain + 时间窗）权重更高
   - 路径类型输出口径不变：初始入侵 → 执行 → 横向移动 → C2 → 外传（由证据决定哪些阶段出现）
4) 输出：
   - 时间线（technique 序列）
...
- 取指定范围内的 **Canonical Findings**（`event.dataset="finding.canonical"`）抽取 technique 集合 `S`，并生成向量表示 `V_attack`
- 取离线 MITRE ATT&CK Enterprise CTI（STIX 2.1｜attack-stix-data｜默认路径：`backend/app/services/ttp_similarity/cti/enterprise-attack.json`，可用 `ATTACK_CTI_PATH` 覆盖）数据包中的 Intrusion Set（组织）→ technique 集合，生成每个组织的向量表示 `V_i`（数据文件可用 `backend/scripts/fetch_mitre_attack_cti.sh` 获取/更新）。
- 对 technique 做 IDF 加权（稀有 technique 权重更高），得到 `V_attack_tfidf` 与 `V_i_tfidf`
- 相似度使用余弦相似度：`cosine(V_attack_tfidf, V_i_tfidf)`
- 相似度计算在中心机分析模块中完成（TF-IDF + cosine，Top-3 排序），不依赖 Neo4j GDS
...
**后端（Python FastAPI）**：
1) **客户端对接接口（注册 + 拉取）**：见 `docs/06C-客户端中心机接口规范.md`
2) **数据查询与分析接口**：供前端调用的 RESTful API
...
- `POST /api/v1/events/search` - 事件检索
- `POST /api/v1/findings/search` - 告警检索
- `POST /api/v1/analysis/ttp-similarity` - APT/TTP 相似性匹配（Top-3 + 解释性 techniques）
- `POST /api/v1/chains/generate` - 生成攻击链
- `GET /api/v1/chains/{id}` - 攻击链详情
