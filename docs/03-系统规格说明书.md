...
- 取本次攻击链 technique 集合 `S`，并生成向量表示 `V_attack`
- 取离线 ATT&CK CTI（STIX）数据包中的 Intrusion Set（组织）→ technique 集合，生成每个组织的向量表示 `V_i`
- 对 technique 做 IDF 加权（稀有 technique 权重更高），得到 `V_attack_tfidf` 与 `V_i_tfidf`
- 相似度使用余弦相似度：`cosine(V_attack_tfidf, V_i_tfidf)`
- 相似检索在 Neo4j GDS 中完成（基于向量的 Top-K 相似度检索）

输出建议：

- Top-3 候选组织 + 相似度分数
- 贡献度最高的 3–5 个 techniques（作为解释）：按 `V_attack(t) * V_i(t) * idf(t)` 排序取 Top
