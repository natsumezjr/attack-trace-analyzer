...
客户机侧对外提供拉取接口时，必须为每条返回的 ECS 文档生成 `event.id`，规则固定为：

- `event.id = "evt-" + sha1(client_id + ":" + source_table + ":" + row_id)[:16]`

其中：

- `client_id`：客户机注册使用的 ID（见 `54-客户机-中心机接口规范.md`）
- `source_table`：数据来源表名（`filebeat`/`falco`/`suricata`）
- `row_id`：SQLite 自增主键 `id`

> 说明：此规则保证跨次拉取稳定、无需解析原始日志内容即可生成幂等键。
