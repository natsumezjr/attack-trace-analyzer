...
1. 应用启动时自动启动后台轮询任务（`start_polling()`）
2. 应用关闭时优雅停止轮询任务（`stop_polling()`）
3. 轮询任务在后台独立线程运行，不阻塞 HTTP 请求处理
...
**文件：** `backend/app/services/ttp_similarity/router.py`
...
1. 从 `canonical-findings-*` 提取 `threat.technique.id`
2. 过滤占位符（T0000、UNKNOWN、TBD）
3. 加载 Enterprise CTI 数据（`enterprise-attack.json`）
4. 计算 TF-IDF + 余弦相似度
5. 返回 Top-3 相似 APT 组织
...
- `501`：模块导入失败（见 `router.py` 的 try-except）
...
| `ATTACK_CTI_PATH` | `backend/app/services/ttp_similarity/cti/enterprise-attack.json` | CTI 文件路径 |
...
### 5.3 Analysis 模块集成（可选）

**TTP 相似度模块：** `backend/app/services/ttp_similarity/`
...
**调用位置：**
- `ttp_similarity/router.py`：`POST /api/v1/analysis/ttp-similarity`

**CTI 数据：**
- 文件：`enterprise-attack.json`（MITRE ATT&CK Enterprise STIX Bundle）
- 加载：`@lru_cache(maxsize=1)` 单例缓存
- 索引结构：`EnterpriseCtiIndex`（intrusion_sets / techniques / uses / idf / group_norm）
...
try:
    ...
    def ttp_similarity_unavailable():
        return JSONResponse(
            status_code=501,
            content=err("NOT_IMPLEMENTED", "ttp similarity unavailable"),
        )
```

### 8.2 Neo4j 不可用

**响应：** 501 NOT_IMPLEMENTED

**场景：** Neo4j 未安装 / 连接失败 / GDS 插件未启用

**前端处理：** 禁用图可视化功能，但不影响事件/告警检索

### 8.3 OpenSearch 不可用

**响应：** 503 SERVICE_UNAVAILABLE

**场景：** 连接超时 / 认证失败 / 索引不存在

**影响范围：** 事件检索、告警检索、TTP 相似度
...
| `backend/app/services/ttp_similarity/router.py` | TTP 相似度路由 |
| `backend/app/services/ttp_similarity/service.py` | TTP 相似度服务 |
