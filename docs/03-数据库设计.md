...
- **跨源关联优先**：字段设计围绕 host/user/process/ip/domain/file 等实体展开（session 为派生概念）。
...
- `Entity`：抽象实体（host/user/process/file/ip/domain）
...
- `event.original`：text（建议保留，便于证据回放/截图；必要时可截断）
...
- `ata.evidence.event_ids`：keyword[]（证据事件 `event.id` 列表，`ata.*` 自定义字段）
- `ata.evidence.query`：text（预留：v1 不强制，不作为实现目标）
...
- `ata.chain.apt_topn`：object（APT/TTP 相似度候选列表）
