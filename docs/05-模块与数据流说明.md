...
- **Wazuh（主机日志）**：回答"谁在什么时候登录、创建了什么进程、权限是否变化"
...
**组件**：Next.js API Routes

**输入**：客户端注册请求、拉取响应

**输出**：写入 OpenSearch 的数据
...
**组件**：Python 算法模块（驱动 Neo4j GDS，消费离线 ATT&CK CTI，融合 YARA 指纹）
...
- **APT 相似度匹配**：基于离线 ATT&CK CTI（Intrusion Set ↔ Technique）+ TF-IDF 向量 + 余弦相似度，在 Neo4j GDS 中输出 Top-3 候选组织与解释性 techniques

---

### 2.6 模块 6：图谱与展示层

**组件**：Neo4j + Next.js Frontend
...
- **Neo4j**：作为实体关系图的权威存储，用于多跳路径推理与图展示
- **Next.js UI**：
  - 时间线视图：按时间展示攻击各阶段
...
接收客户端注册，维护客户端清单（`client_id/listen_url/capabilities/last_seen/cursor` 等）

**2) 轮询器（Poller）**

按 interval 轮询客户端，拉取新事件/告警

**3) 入库与索引**

写入 OpenSearch（Telemetry/Findings/Chains…）并驱动后续检测/关联
...
- 取本次攻击链 technique 集合 `S` 并生成向量 `V_attack`
- 取离线 ATT&CK CTI（STIX）数据包中的 Intrusion Set → technique 集合，生成每个组织的向量 `V_i`
- 对 technique 做 IDF 加权（稀有 technique 权重更高），得到 `V_attack_tfidf` 与 `V_i_tfidf`
- 相似度使用余弦相似度：`cosine(V_attack_tfidf, V_i_tfidf)`
- 相似检索在 Neo4j GDS 中完成，输出 Top-3 候选组织
...
Wazuh/Falco/Suricata（采集）
  ...
  → 中心机 Next.js（注册表 + 轮询拉取）
  → OpenSearch（ecs-events-* / raw-findings-* / canonical-findings-*）
  → Security Analytics + Sigma（Store-first 检测）
  → 告警融合（Raw → Canonical）
  → Python 关联算法（实体抽取 + GDS 加权最短路串链 + TTP 相似度 + YARA 指纹汇总）
  → Neo4j + GDS（实体关系图 + 图算法）
  → Next.js 前端（时间线 / 关系图 / 攻击链 / 报告导出 / Navigator layer）
```
...
| 步骤       | 技术                       | 产出                        |
| ---------- | -------------------------- | --------------------------- |
| 采集       | Wazuh / Falco / Suricata   | 原始日志 + 告警             |
| 客户端后端 | Go + SQLite                | ECS 事件 + 告警             |
| 中心机后端 | Next.js                    | API + 注册表                |
| 存储       | OpenSearch                 | 3 个索引                    |
| 检测       | Security Analytics + Sigma | Raw Findings                |
| 分析       | Python + Neo4j GDS + YARA + 离线 ATT&CK CTI | Canonical Findings + 攻击链 + 相似 APT 候选 + 指纹证据 |
