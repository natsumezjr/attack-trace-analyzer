...
1) 中心机读取注册表条目（含 `listen_url`、`client_token_hash`、`poll.cursor`）。  
2) 调用客户机拉取接口（见 `54-客户机-中心机接口规范.md`），并携带 Bearer token。  
3) 客户机返回：
   - ECS 事件列表（Telemetry 与 Raw Finding 允许混合返回）
   - 新的游标 `next_cursor`

中心机必须把 `next_cursor` 写回注册表，作为下次拉取的起点。
