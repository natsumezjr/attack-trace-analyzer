...
- `data[]` 的每个元素必须为合法 JSON 对象（dict），允许两种字段形态：嵌套对象 / 点号扁平键（见 `51-ECS字段规范.md` 0.6）。
- 客户机侧对外返回前必须保证 **稳定 `event.id`**：
  - 当上游消息已包含 `event.id` 时透传；
  - 当缺失 `event.id` 时，按 `51-ECS字段规范.md` 0.5 规则生成：`evt-` + `sha1(raw_payload_bytes)[:16]`。
- 其余公共字段由中心机在 Step 2 入库前执行“最终规范化”（见 `31-OpenSearch模块规格说明书.md` 3.2、`51-ECS字段规范.md` 0.4/0.6）：
