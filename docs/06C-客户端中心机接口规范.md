...
客户端同时执行 **YARA 指纹扫描**：对落地文件/脚本/配置进行规则匹配，产出 Finding（`event.kind=alert`，`event.dataset=finding.raw`，`rule.ruleset=yara`），用于“攻击者指纹”证据输出。
...
| **指纹扫描** | 对落地文件/脚本/配置做特征匹配，产出 YARA Findings | YARA |
...
### 2.2 中心机（Center Backend / FastAPI）

中心机后端由 **Python FastAPI** 承担。
...
| **轮询器** | 定时轮询客户端拉取事件/告警 | 定时任务 + HTTP 客户端（APScheduler/aiohttp） |
| **入库** | 写入 OpenSearch/Neo4j | OpenSearch Client + Neo4j Driver |
| **关联算法** | 告警融合、时间窗关联、路径重建 | Python 算法模块（直接调用，无需跨语言） |
| **攻击链构建** | 生成攻击链对象 | 关联算法输出 |
| **API 服务** | 提供RESTful API 供前端调用 | FastAPI 路由 |

**前后端分离**：

- **后端**：Python FastAPI，提供 API 服务
- **前端**：Next.js + React，通过 API 调用后端服务
...
| `capabilities.yara` | `boolean` | ✅ | 能力：YARA 指纹扫描 | `true` |
...
- `want.*` 为过滤字段；**本项目 v1 支持并按其进行过滤**（缺省表示不过滤：返回全部）
...
| `last_error` | `text` | 最后错误（如有） | `Connection refused` |
