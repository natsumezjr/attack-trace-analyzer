...
>   `docs/06A-ECS字段规范.md`
> - **03B**：存储与图谱设计（OpenSearch 索引 + Neo4j 图模型 + 关系型表结构用于报告展示）
>   `docs/06B-存储与图谱设计.md`
...
- 任意边都能回放证据（边属性保存 `custom.evidence.event_ids`）
- 用于路径重建与图展示，并在 Neo4j GDS 中运行加权最短路（路径重建）
...
> 关系命名与抽取口径以：`docs/09-实体图谱节点与关系规范v2.md` 为准（v2 规范）。
...
**定义**：链路摘要、步骤、证据引用与相似度输出。

**特点**：
- 量最小（最高层抽象）
- 展示价值最高（可直接用于报告）
- 包含攻击阶段（`Initial Access` → `Execution` → ...）
- 包含关键证据（`custom.evidence.event_ids`）
- 包含溯源增强输出：相似 APT 候选（Intrusion Set Top-3 + 解释性 techniques）
...
|---------|---------|---------|------|
| **Filebeat** | 主机系统日志 | `hostlog.*` | 认证/登录、进程事件、系统审计日志 |
...
**职责**：
1. **注册表**：维护客户端清单与轮询 cursor（落 OpenSearch，索引 `client-registry`；注册表不按天拆分）
...
- `client-registry` 为中心机元数据索引，不做按天拆分
- 不同生命周期的数据可设置不同的保留策略（ISM / Index Management policy）
...
- 边（关系）：以 `docs/09-实体图谱节点与关系规范v2.md` 为准（`LOGON`, `RUNS_ON`, `SPAWN`, `FILE_ACCESS`, `NET_CONNECT`, `DNS_QUERY`, `RESOLVES_TO`, `HAS_IP`）
- 边属性：`custom.evidence.event_ids`（指向 Telemetry 的 `event.id`）
...
| 字段 | 用途 | 生成策略 |
|------|------|---------|
| `event.id` | 全局幂等键 | `sha1(agent.name + raw_unique_key + @timestamp)` |
| `host.id` | 跨源最强关联键 | `sha1(host.name)` |
