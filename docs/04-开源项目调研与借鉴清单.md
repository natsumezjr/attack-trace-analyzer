...
- 攻击者指纹：YARA（离线规则库，输出为 Finding 并进入报告）
- TTP 知识库：离线 ATT&CK CTI（STIX）数据包（Intrusion Set ↔ Technique）
...
- 离线 ATT&CK CTI（STIX）数据包：提供 Intrusion Set ↔ Technique 映射，用于 TTP 相似性匹配
- 基于 Suricata 的 DNS/流量观测：构建域名 ↔ IP ↔ 会话的基础设施关联证据
- YARA：对落地样本/脚本/配置进行特征匹配，形成“攻击者指纹”证据并进入报告
...
- 统一字段：ECS（统一范式）
- 图谱与图算法：Neo4j + GDS（加权最短路、相似度检索）
- 指纹：YARA（离线规则库）
- 知识库：离线 ATT&CK CTI（STIX）
