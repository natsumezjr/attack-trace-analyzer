...
本文档是《攻击溯源分析系统》的详细设计总文档，整合了客户机、中心机、分析模块、前端模块四大组件的详细设计内容，为系统开发提供具体的技术实现指导。
...
**技术栈总览**：
- **客户机**：Gin (Go) + Falco + Filebeat + Suricata + RabbitMQ
- **中心机**：FastAPI (Python) + OpenSearch + Neo4j
- **分析模块**：Python + NetworkX + LLM (DeepSeek)
- **前端**：Next.js (React) + G6 图可视化库
...
### 二、客户机模块
- [1. 客户机职责边界](#1-客户机职责边界)
- [2. 组件清单与目录结构](#2-组件清单与目录结构)
- [3. 端到端数据流](#3-端到端数据流)
  - [3.1 数据流图（完整视图）](#31-数据流图完整视图)
  - [3.2 三传感器数据流对比](#32-三传感器数据流对比)
  - [3.3 关键设计约束](#33-关键设计约束)
- [4. 拉取接口设计](#4-拉取接口设计)
- [5. 队列语义与消息模型](#5-队列语义与消息模型)
...
- [2. 注册与轮询](#2-注册与轮询)
- [3. OpenSearch 存储与索引治理](#3-opensearch-存储与索引治理)
- [4. 检测与告警融合](#4-检测与告警融合)
- [5. Neo4j 入图与图查询](#5-neo4j-入图与图查询)
- [6. 图谱回标与边属性](#6-图谱回标与边属性)

### 四、分析模块
- [1. 任务模型与状态机](#1-任务模型与状态机)
- [2. 候选路径构造与评分](#2-候选路径构造与评分)
- [3. LLM 选择器与回退机制](#3-llm-选择器与回退机制)
- [4. TTP 相似度匹配](#4-ttp-相似度匹配)

### 五、前端模块
- [1. 总体与页面结构](#1-总体与页面结构)
- [2. 图谱可视化与交互](#2-图谱可视化与交互)
- [3. 报告导出](#3-报告导出)
