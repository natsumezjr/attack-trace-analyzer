...
本文档是《攻击溯源分析系统》的概要设计总文档，整合了系统架构、数据流、非功能设计等核心内容，旨在为系统实现提供顶层设计指导。
...
**设计亮点**：
- **单定时器顺序流水线架构**：确保数据一致性与可追溯性
- **五层数据分层模型**：从原始事件到溯源结果的清晰演进路径
- **异步溯源任务机制**：支持长时间图分析而不阻塞实时查询
- **前端可视化闭环**：图谱交互、告警检索、溯源报告一体化体验
...
- [一、概要设计报告](#一、概要设计报告)
  - [1. 总体架构](#1-总体架构)
    - [1.1 术语与缩略语](#11-术语与缩略语)
    - [1.2 系统组成](#12-系统组成)
    - [1.3 系统架构图](#13-系统架构图逻辑视图)
  - [2. 核心数据分层](#2-核心数据分层中心机视角)
    - [2.1 数据分层架构](#21-数据分层架构)
    - [2.2 数据层次说明](#22-数据层次说明)
  - [3. 中心机定时流水线](#3-中心机定时流水线单定时器顺序流水线)
    - [3.1 调度原则](#31-调度原则)
    - [3.2 设计决策](#32-设计决策为什么采用单定时器顺序流水线架构)
    - [3.3 流水线输入与输出](#33-流水线输入与输出)
    - [3.4 四步顺序与职责](#34-四步顺序与职责每次-tick-内严格顺序执行)
    - [3.5 流水线时序图](#35-流水线时序图)
  - [4. 前端可视化查询链路](#4-前端可视化查询链路)
  - [5. 异步溯源任务](#5-异步溯源任务create_task)
  - [6. 安全与审计边界](#6-安全与审计边界课程演示)
- [二、数据流与时序](#二-数据流与时序)
  - [1. 数据对象分类](#1-数据对象分类)
  - [2. 端到端数据流](#2-端到端数据流中心机单定时器流水线)
  - [3. 前端可视化数据流](#3-前端可视化数据流)
  - [4. 溯源任务数据流](#4-溯源任务数据流)
- [四、非功能设计](#四-非功能设计)
  - [1. 幂等与去重](#1-幂等与去重)
  - [2. 一致性与可回放](#2-一致性与可回放)
  - [3. 可解释与证据链](#3-可解释与证据链)
  - [4. 性能目标与容量边界](#4-性能目标与容量边界)
  - [5. 稳定性与故障处理](#5-稳定性与故障处理)
...
2) 客户机侧把采集结果转换为 ECS 文档（字段口径见 `../80-规范/81-ECS字段规范.md`）。
...
上述四步由同一个定时器驱动，形成"顺序流水线"，系统规格见 `40-概要设计报告.md`。
