...
本文件用于答辩 PPT 的逐页文案与配图规划，口径与结构对齐《PPT大纲与演示脚本》。
...
- 靶场与 C2（`deploy/` + 《程序编译和安装使用文档》）：6 节点固定拓扑，C2 提供 DNS/HTTP 证据点。
...
> 截图命名规则：`YYYYMMDD-序号-证据主题.png`（见《录像与截图清单》）

| ID | 用途页 | 内容 | 来源/生成方式 | 备注 |
|---|---|---|---|---|
| V1 | S1 | 课程要求 4 块对标图 | 自绘（四象限/流程图） | 强调对标与目标 |
| V2 | S2 | 系统架构图 | 现有图（文件名：`68282b25772f2821f09fe07657065c8aa00a56f30f9a7a533ee9f975278b66b7.jpg`） | 现有图 |
| V3 | S2 | 6 节点靶场拓扑图 | 自绘，参考《程序编译和安装使用文档》靶场部署部分 | center-01/client-01..04/c2-01 |
| V4 | S3 | 采集链路图 | 自绘，参考《详细设计报告》客户机章节 | 3 传感器→ECS→MQ→API |
| V5 | S3 | 客户机 API `total>0` | 按验证清单执行 curl | 选 1–2 张截图 |
