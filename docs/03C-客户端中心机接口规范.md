...
> **本文定位**：本文定义 **客户端后端（Client Backend）** 与 **中心机后端（Center Backend / Next.js）** 的交互协议：
> - 客户端启动后 **主动向中心机注册**
> - 注册成功后，中心机 **轮询客户端** 拉取数据（Telemetry/Findings）
>
> **相关文档**：
> - 字段规范（ECS 子集 + `custom.*`）：`docs/03A-ECS字段规范.md`
> - 存储与图谱设计（OpenSearch + Neo4j）：`docs/03B-存储与图谱设计.md`
...
| **归一化** | 将原始事件映射为 ECS 子集字段 | 见 `docs/03A-ECS字段规范.md` |
| **缓冲** | 本地缓冲，避免数据丢失 | 队列/文件/SQLite |
| **注册** | 启动后主动向中心机注册 | `POST /api/v1/clients/register` |
| **拉取接口** | 提供 HTTP API，供中心机轮询拉取 | `POST /api/v1/pull` |
| **健康检查** | 提供健康检查接口（可选） | `GET /api/v1/health` |
...
| 字段 | 类型 | 必填 | 说明 | 示例 |
|------|------|:----:|------|------|
| `cursor` | `string` | ✅ | 上次拉取位置（字符串即可） | `"0"`, `"500"` |
| `limit` | `integer` | ✅ | 分页大小（建议 200–2000） | `500` |
| `want.event_kinds` | `string[]` | ⭐ | 想要的事件类型（`event`/`alert`） | `["event", "alert"]` |
| `want.datasets` | `string[]` | ⭐ | 想要的数据集（支持通配符） | `["hostlog.*", "netflow.dns"]` |

**cursor 的实现方式**：
- 本地队列 offset（如 Kafka offset）
- SQLite 自增 id
- 文件行号
- 时间戳

**约束**：
- `limit` 不宜过大，避免单次响应过大（建议 ≤ 2000）
- `want.*` 为可选字段，MVP 可忽略（返回所有数据）
...
|------|------|------|------|
| `status` | `string` | 状态，固定为 `ok` | `ok` |
| `items` | `object[]` | ECS 事件数组（见 `docs/03A-ECS字段规范.md`） | - |
...
建议中心机对每个客户端维护以下状态（可存在 OpenSearch/关系表/内存）：
...
| 参数 | 推荐值 | 说明 |
|------|-------|------|
| `poll_interval_seconds` | 5–15 秒 | 轮询间隔（演示优先"实时可见"） |
...
- **存储与图谱设计**：`docs/03B-存储与图谱设计.md`
- **数据设计总览**：`docs/03-数据库设计.md`
- **模块与数据流说明**：`docs/04-模块与数据流说明.md`
- **系统规格说明书**：`docs/02-系统规格说明书.md`
