...
- 自定义字段：统一使用命名空间 **`custom.*`**
- 图谱：**Neo4j**（实体关系图的权威存储）
- 主机侧（Linux 为主）：
  - 主机日志：**Wazuh**（主机日志采集只使用 Wazuh）
  - 主机行为：**Falco**（行为告警为主；Docker quickstart：`https://falco.org/docs/getting-started/falco-docker-quickstart/`）
...
- ECS + `custom.*`：保证跨源字段一致，同时避免自定义字段与 ECS 冲突。
