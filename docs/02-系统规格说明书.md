...
系统由“采集与入库”和“分析与输出”两大部分组成。前者尽量复用开源组件；后者（关联、串链、相似度）作为课程项目的核心自研点。

### 1.1 组件分层

1) **采集层（Collectors）**  
主机日志、主机行为、网络流量分别由对应传感器采集。

2) **归一化层（Normalization / ECS Mapping）**  
把不同来源事件映射到 ECS 子集字段，形成可跨源关联的统一事件。

3) **存储与检索层（Storage）**  
以 **OpenSearch** 为核心存储：分别保存 Telemetry、Findings 与分析产物。

4) **检测层（Detection）**  
既支持传感器直接产告警（Detect-first），也支持入库后扫描产告警（Store-first）。

5) **关联与溯源层（Correlation & Tracing）**  
从 Findings/Telemetry 抽取实体与关系，写入 **Neo4j** 实体关系图；在图上完成路径重建与攻击链生成，并将攻击链摘要写回 OpenSearch 供检索与展示。

## 2. 数据对象：从大到小的“金字塔”

为避免“统一进 OpenSearch 后还会产出什么数据”的混乱，本项目定义 4 类核心数据对象：
...
- 主机日志（Linux 为主）：`auditd` + `auth.log`（SSH/sudo 等）
- 主机行为：Falco（输出行为告警为主）
- 网络流量：Zeek（事实协议日志） + Suricata（告警）
...
- 主机：`host.name`（或唯一标识）
- 用户（如适用）：`user.name`、`user.id`（或 logon/session 标识）
...
- 回溯：`event.original`（或 `raw_ref` / `related.event.id`）
...
- `ata.finding.id`：唯一 ID
- `ata.finding.stage`：`raw` / `canonical`
- `ata.finding.providers`：告警来源列表（raw 可为单元素；canonical 为多元素）
- `@timestamp`：命中时间（或检测时间）
- `rule.id` / `rule.name` / `rule.version`：规则信息（ECS）
- `event.severity`：严重度（统一为 0–100 的整数，ECS）
- `ata.confidence`：可信度（0–1）
...
- `related.event.id`：证据引用（命中的 Telemetry 事件 ID 列表，ECS）
- `ata.evidence.query`：可选，用于回放证据的 OpenSearch 查询/条件

> 统一 Finding 模型是本项目最关键的“胶水层”。它决定了后续能否用一套关联逻辑处理不同来源告警。
>
> 说明：本项目以 ECS 为主（`event.* / rule.* / threat.* / host.* ...`），所有自定义字段统一放入 `ata.*` 命名空间（见 `docs/99-选型决策记录.md`）。
...
- `ata.finding.providers`：来源引擎追加（suricata/falco/opensearch-security-analytics…）
- `related.event.id`：证据引用合并去重
- `event.severity`：取 max 或加权
- `ata.confidence`：可按“来源数量/一致性”上调
...
每条边必须携带 `ata.evidence_event_ids`，能回溯到 Telemetry/Findings。
...
可按实际实现选择 REST/CLI。REST 示例：

- `POST /api/v1/ingest`：导入离线数据包（可选）
- `GET /api/v1/events`：查询 Telemetry（按时间/host/实体过滤）
- `GET /api/v1/findings`：查询 Findings（按 technique/tactic/规则过滤）
- `POST /api/v1/chains/build`：按时间窗口构建攻击链
- `GET /api/v1/chains/{chain_id}`：获取链路详情（图 + 时间线 + 摘要）
- `GET /api/v1/chains/{chain_id}/export/navigator`：导出 ATT&CK Navigator layer

### 6.2 输出物清单

- `Attack Chain Report`：链路摘要（markdown/pdf 均可）
