...
- `ata.evidence.event_ids`：证据引用（命中的 Telemetry `event.id` 列表，`ata.*` 自定义字段）
- `ata.evidence.query`：预留字段（v1 不作为实现目标）
...
- `ata.evidence.event_ids`：证据引用合并去重
...
本项目采用 **CLI 驱动**（课程项目无需部署长期运行的 API 服务）。建议命令规格：

- `ata ingest`：导入离线数据包到 OpenSearch（用于演示/复现实验批次）
- `ata query events`：查询 Telemetry（按时间/host/实体过滤）
- `ata query findings`：查询 Findings（按 technique/tactic/规则过滤）
- `ata build-chain`：按时间窗口构建攻击链
- `ata show-chain`：获取链路详情（图 + 时间线 + 摘要）
- `ata export navigator`：导出 ATT&CK Navigator layer
