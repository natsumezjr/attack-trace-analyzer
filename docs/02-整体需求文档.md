...
- 输出可用于汇报/答辩的证据链（时间线、实体关系图、链路摘要、相似 APT 候选解释）。

## 2. 范围与边界

### 2.1 在范围内（In Scope）

- 多源数据的采集接入（Filebeat(+Sigma)/Falco/Suricata）
- 客户端后端（Go）：归一化 + **SQLite 本地缓冲** + 向中心机注册 + 提供拉取接口（中心机轮询）
- 中心机后端（Python FastAPI）：客户端注册表 + 轮询拉取 + 入库 OpenSearch/Neo4j + API 服务
- 中心机前端（Next.js + React）：前端页面展示、可视化、报告导出
- 时间对齐、字段范式化（统一到 ECS 子集）
- 关键实体抽取（host/user/process/file/ip/domain；session 为派生概念）
- 检测结果（Findings/Alerts）汇聚与统一
- 多源关联：时间窗口聚合 + 实体关系图构建
- 攻击链重建：基于 Neo4j GDS 的加权最短路输出关键路径（初始入侵/横向移动/外传）
- APT/TTP 相似性匹配（Top-N 候选 + 解释性证据）：基于离线 MITRE ATT&CK Enterprise CTI（STIX 2.1｜attack-stix-data，Intrusion Set ↔ Technique）+ TF-IDF + 余弦相似度
- 靶场验证（≥5 节点）与演示材料输出
...
- 外部互联网情报查询（如 WHOIS/RDAP/Passive DNS 等）：中心机运行环境离线，不依赖外部网络数据源

## 3. 术语与定义

- **Telemetry（事实事件）**：未下结论的观测数据，如 Suricata EVE JSON（flow/dns/http…）、Filebeat 采集的主机系统日志等。
- **Finding/Alert（检测结果）**：规则或模型对 Telemetry 的判定结果（含严重度、规则、ATT&CK 标注）。
- **ECS（Elastic Common Schema）**：用于跨源字段统一的通用字段集合（本项目采用“ECS 子集”）。
- **Client Backend（客户端后端 / Go）**：运行在客户机侧的轻量服务：汇聚三路采集输出，归一化为 ECS 子集并 **SQLite 本地缓冲**；启动后向中心机注册；等待中心机轮询拉取数据。
- **Center Backend（中心机后端）**：运行在中心机侧的服务（本项目用 Python FastAPI）：维护客户端注册表与轮询状态，拉取数据入库，并运行关联算法生成攻击链，提供 RESTful API 供前端调用。
- **Center Frontend（中心机前端）**：运行在中心机侧的前端服务（本项目用 Next.js + React）：通过 API 调用后端服务，提供页面展示、可视化、报告导出等功能。
...
- 支持接入三类数据源：
  - 主机系统日志（Filebeat；可结合 Sigma 规则检测异常）
...
### FR-03 中心机服务（FastAPI 后端 + Next.js 前端）

- 中心机后端必须维护客户端注册表与轮询状态（cursor/last_seen/last_error）。
- 中心机前端必须提供前端页面，展示：
  - 事件检索、告警检索
  - 攻击链与证据回放
  - 关系图展示
...
- 支持两种告警产生模式：
  - **Detect-first**：传感器侧直接产生告警（如 Filebeat+Sigma / Falco / Suricata）
...
- 路径重建基于 Neo4j 实体图，使用 Neo4j GDS 加权最短路算法生成关键路径，并能回溯每条边的证据。

### FR-12 APT/TTP 相似性匹配（候选）

- 基于指定范围内的 **Canonical Findings**（`event.dataset="finding.canonical"`）抽取 technique 集合，与已知 **ATT&CK Intrusion Set（组织）** 的 technique 集合做相似度比较。
- 已知组织的 technique 集合来自 **离线 MITRE ATT&CK Enterprise CTI（STIX 2.1｜attack-stix-data｜默认路径：`backend/app/services/ttp_similarity/cti/enterprise-attack.json`，可用 `ATTACK_CTI_PATH` 覆盖）** 数据包；系统运行时不依赖联网查询。数据文件可通过根目录脚本 `backend/scripts/fetch_mitre_attack_cti.sh` 获取/更新。
- 相似度使用 **TF-IDF 加权的余弦相似度**，由中心机分析模块做 Top-3 排序输出（不依赖 Neo4j GDS）。
- 输出 Top-3 候选及分数，并给出“贡献度最高的 5 个 techniques”作为解释性证据。
...
- 中心机运行环境离线：ATT&CK 知识库以离线 MITRE ATT&CK Enterprise CTI（STIX 2.1｜attack-stix-data）数据包形式随项目交付与导入，系统运行时不依赖外部互联网数据源。
