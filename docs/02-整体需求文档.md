...
- 已知组织的 technique 集合来自 **离线 MITRE ATT&CK Enterprise CTI（STIX 2.1｜attack-stix-data｜默认路径：`backend/app/services/ttp_similarity/cti/enterprise-attack.json`，可用 `ATTACK_CTI_PATH` 覆盖）** 数据包；系统运行时不依赖联网查询。数据文件可通过根目录脚本 `./scripts/fetch_attack_cti.sh` 获取/更新。
