...
1. 创建溯源任务并等待完成。
2. 调用 `GET /api/v1/analysis/tasks/{task_id}` 获取任务文档。
3. 检查任务结果字段存在且可读：
...
4. 检查 `task.result.ttp_similarity.similar_apts` 至少包含 1 项，且每项包含：
   - `intrusion_set.id`
   - `intrusion_set.name`
   - `similarity_score`
   - `top_tactics`
...
- 任务文档中 `task.result.ttp_similarity.*` 字段截图

**失败判定**

- `task.result.ttp_similarity.*` 任一字段缺失
- `task.result.ttp_similarity.similar_apts` 为空数组
