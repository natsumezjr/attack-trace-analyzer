...
中心机以 OpenSearch 索引 `client-registry` 作为客户机注册表的唯一权威数据源，维护客户机连接信息与轮询状态。
...
- `poll.last_seen`：最近一次成功轮询的时间戳
- `poll.status`：最近一次轮询状态（success/error/partial_success）
- `poll.last_error`：最近一次轮询失败的错误信息

字段结构与索引约束由 `../../80-规范/82-OpenSearch索引与Mapping规范.md` 与 `../../80-规范/87-客户机与中心机接口.md` 定义。

## 2. 注册流程

中心机通过 `POST /api/v1/clients/register` 接口接收客户机注册请求。注册成功后必须将客户机信息写入 `client-registry` 索引，确保注册信息持久化。

接口请求与响应的权威定义见 `../../80-规范/87-客户机与中心机接口.md`。

注册失败时严禁降级为"仅内存登记"，必须返回错误响应并保持注册表不变，确保注册状态的可靠性与一致性。
...
1. **注册表读取**：从 OpenSearch 读取 `client-registry` 索引获取已注册客户机列表；
2. **客户机遍历**：逐客户机解析 `client.id`、`client.listen_url`、`client.capabilities` 字段；
3. **路由选择**：根据 capabilities 声明确定拉取路由（`falco`、`suricata`、`filebeat` 的子集）；
4. **HTTP 拉取**：向每个路由发送 HTTP GET 请求 `{listen_url}/{route}` 拉取数据；
5. **事件提取**：提取响应体中的 `data[]` 数组并汇总为本 tick 的事件总列表；
6. **状态更新**：将轮询结果写回注册表的 `poll.*` 字段（`last_seen`、`status`、`last_error`）；
7. **事件存储**（Step 2）：调用 `store_events()` 将事件列表写入 OpenSearch，按 `event.kind` 路由至 Telemetry 或 Raw Findings 索引；
8. **数据分析**（Step 3）：调用 `run_data_analysis()` 触发 Security Analytics 检测与告警融合，生成 Canonical Findings 并写回 OpenSearch；
9. **图谱入库**（Step 4）：调用 Neo4j 入图流程，将 Telemetry 与 Canonical Findings 转换为节点与边写入图谱。
...
轮询服务对每个客户机执行以下错误处理策略：

1. **数据源隔离**：任一数据源拉取失败不影响其他数据源的拉取；
2. **客户机隔离**：任一客户机轮询失败不阻塞其他客户机的轮询；
3. **状态记录**：失败信息写入 `poll.last_error`，状态标记为 `poll.status=error`，下一轮自动重试；
4. **任务持续性**：轮询循环永不因单个客户机异常而退出；
5. **必选依赖失败处理**：OpenSearch 与 Neo4j 为中心机必选依赖，当 Step 3 或 Step 4 失败时，轮询任务应快速失败并抛出异常（便于靶场联调及时发现问题），但严禁在后台任务中直接调用 `os._exit(1)` 杀死 FastAPI 进程（避免服务不可控退出，便于排障与恢复）。
...
1. **正常流程**：定时器触发轮询服务 → 遍历所有客户机拉取三个数据源 → 更新注册表状态 → 执行流水线处理（存储/分析/入图）；
2. **超时处理**：单个数据源超时（如 falco）不阻塞其他数据源（suricata/filebeat 继续拉取）→ 记录错误到 `poll.last_error` → 状态标记为 `partial_success` → 继续处理已成功拉取的事件；
3. **流水线故障**：当 OpenSearch/Neo4j 等必选依赖失败时 → 抛出异常停止轮询任务 → 记录完整错误堆栈到日志 → 不杀死 FastAPI 进程（保持服务可用性，便于排障与恢复）。
