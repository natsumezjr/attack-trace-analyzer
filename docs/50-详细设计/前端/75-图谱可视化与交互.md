...
本文档规定图谱可视化的渲染模型、筛选与高亮规则，以及节点交互触发溯源任务的完整流程。
...
图谱页面数据来自中心机图查询接口：`POST /api/v1/graph/query`。

### 1.1 视图模式（固定）

溯源分析页面提供三种视图模式，满足不同分析需求：

1. **告警视图**：`action="alarm_edges"` — 展示 `is_alarm=true` 的边集合
2. **时间窗视图**：`action="edges_in_window"` — 展示用户选定时间范围内的边集合
3. **任务视图**：`action="analysis_edges_by_task"` — 展示溯源任务写回的边集合

### 1.2 时间窗输入（固定）

后端接口时间窗字段格式（详见 `88-前端与中心机接口.md`）：

- `start_ts`：ISO 8601（UTC）
- `end_ts`：ISO 8601（UTC）

前端页面不提供复杂的时间选择器，仅保留"最近 N 分钟"快捷选项：

1. **用户选择**时间长度 `N`（分钟）
2. **触发操作**时（刷新图谱或创建任务），前端以当前时间 `now` 计算：
   - `end_ts = now`
   - `start_ts = now - N minutes`
3. **时间范围应用**于：
   - 时间窗视图查询（`action="edges_in_window"`）
   - 溯源任务创建（`POST /api/v1/analysis/tasks`）

该规则限制图查询规模、确保任务输入与图展示一致，并保证报告导出的可复现性。
...
字段权威定义见：`../../80-规范/88-前端与中心机接口.md`

## 2. 渲染模型

前端图渲染采用 AntV G6（`@antv/g6`），数据模型与样式绑定关系固定。
...
- `id = node.uid`
- `data = node`（完整保留原对象）
- `label`：从 `node.props` 与 `node.uid` 生成（详见 2.2）
...
- **节点详情**：展示 `uid/ntype/key/props`（JSON 格式）
- **边详情**：展示 `src_uid/dst_uid/rtype/props`（JSON 格式）
...
1. **告警边高亮**：`edge.props.is_alarm=true` → 红色加粗
2. **关键路径边高亮**：`edge.props["analysis.is_path_edge"]=true` → 蓝色加粗
3. **普通边**：灰色细线

当同一条边同时满足多个条件时，关键路径样式覆盖告警样式（优先级：关键路径 > 告警 > 普通）。
...
1. 用户在时间窗视图中点击节点
2. 页面提取该节点的 `uid` 作为 `target_node_uid`
3. 使用当前页面时间窗（由"最近 N 分钟"计算）作为 `start_ts/end_ts`
4. 调用 `POST /api/v1/analysis/tasks` 创建任务

### 4.2 轮询策略（固定）

任务创建成功后，页面启动轮询监听执行状态：

- **轮询接口**：`GET /api/v1/analysis/tasks/{task_id}`
- **轮询间隔**：每 1 秒
- **停止条件**：`task.status` 为 `succeeded`（成功）或 `failed`（失败）
...
任务成功完成后（`task.status="succeeded"`）：

1. 调用 `POST /api/v1/graph/query`，参数：
   - `action="analysis_edges_by_task"`
   - `task_id=<task_id>`
   - `only_path=true`
2. 将返回边集合叠加到当前图谱并应用关键路径高亮样式（见 3.2）

任务失败时（`task.status="failed"`），页面展示失败原因（后端返回的 `error.message`）。
