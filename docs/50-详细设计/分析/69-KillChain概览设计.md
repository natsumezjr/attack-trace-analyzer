...
- **溯源任务**：前端触发创建的异步任务容器
- **TTP 相似度**：基于 Canonical Findings 计算与 APT 组织的相似度
- **KillChain**：基于图谱数据重建攻击路径并生成解释
...
系统使用有限状态自动机 (FSA) 识别攻击阶段，状态枚举固定为：
...
1. 从告警边集合（`is_alarm=true`）开始
2. 按 `threat.tactic.id` 将边分组到对应战术状态
3. 按时间顺序对每个战术状态的边排序
4. 为每个战术状态生成一个 Segment，选择入口/出口锚点
5. 计算每个 Segment 的时间范围（`t_start`, `t_end`）
6. 从每个 Segment 中选择 top 6 异常边作为摘要
...
4. **格式**：使用易懂的形式加上括号标注实体
   - 例如：`p_c2 (pid:1234)` 代表 c2 进程
   - 例如：`host_web (host.id:host-001)` 代表 web 主机
   - 例如：`user_admin (user.name:admin)` 代表管理员用户
