...
> **相关文档**：
>
> - 存储与建模（OpenSearch 索引划分、Neo4j 图模型）：`docs/06B-存储与图谱设计.md`
> - 客户端 ↔ 中心机接口：`docs/06C-客户端中心机接口规范.md`
...
- `event.ingested` 由**中心机入库时写入**，即使客户端/上游携带该字段也会被中心侧覆盖
- 若事件缺少 `@timestamp` 且无法从 `event.created` 推导，中心机应丢弃该事件（避免“无时间轴”的数据污染索引）
...
- **OpenSearch 索引命名与映射建议**：见 `docs/06B-存储与图谱设计.md`（mapping 要点、字段类型建议、避免 text 聚合踩坑等）。
- **客户端 ↔ 中心机接口规范**：见 `docs/06C-客户端中心机接口规范.md`（注册、健康检查、数据拉取）。
